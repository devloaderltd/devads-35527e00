...
function CopyField({ value, mono = true }: { value: string; mono?: boolean }) {
  ...
  return (
    <button
      ...
      className={`inline-flex max-w-full items-center gap-1.5 rounded-md bg-muted/60 px-2 py-1 text-xs hover:bg-muted ${mono ? "font-mono" : ""}`}
    >
      <span className="max-w-[200px] truncate sm:max-w-[240px]">{value}</span>
...
export function SeedDemoButton() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      setReveal(true);
      toast.success(`Credentials rotated · ${created} created · ${seeded} listing(s) seeded`);
    } catch (e: any) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[85vh] max-w-md overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Rotates passwords to fresh 24-character strings on every run, confirms emails, and reseeds
            sample listings if needed. The new passwords are shown once below — copy them now.
          </DialogDescription>
        </DialogHeader>
              ...
              {rotated.map((a) => (
                <div key={a.email} className="flex flex-wrap items-center gap-2 text-sm">
                  ...
                  {reveal ? (
                    ...
                    <span className="rounded-md bg-muted/60 px-2 py-1 font-mono text-xs text-muted-foreground">
                      ••••••••••••••••••••••••
                    </span>
                  )}
                </div>
