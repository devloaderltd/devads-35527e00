  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button asChild><Link to="/post">+ New listing</Link></Button>
      </div>
      {isLoading ? (
        <div className="text-muted-foreground">Loading…</div>
      ) : !data?.length ? (
        <div className="rounded-xl border bg-card p-10 text-center text-muted-foreground">
          You don't have any listings yet. <Link to="/post" className="text-primary hover:underline">Post your first one</Link>.
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
          {data.map((l: any) => (
            <div key={l.id} className="flex flex-col gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => remove(l.id)}>Delete</Button>
            </div>
          ))}
        </div>
      )}
