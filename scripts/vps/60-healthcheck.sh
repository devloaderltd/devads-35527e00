...
CHECK=app_local; echo "== 7. app :3000"
...
CHECK=studio_gated; echo "== 9. studio access rule"
code=$(curl -ks -o /dev/null -w "%{http_code}" "https://studio.${DOMAIN}/")
case "$code" in
  401|403) ok "studio is gated -> $code" ;;
...
metric_emit supabase_healthcheck_passed "$PASS" 2>/dev/null || true
metric_emit supabase_healthcheck_failed "$FAIL" 2>/dev/null || true
metric_emit supabase_healthcheck_last_run "$(date +%s)" 2>/dev/null || true
[ "$FAIL" -eq 0 ]
